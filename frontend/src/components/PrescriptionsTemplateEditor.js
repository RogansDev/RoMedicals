...
const PrescriptionsTemplateEditor = ({ specialty, onClose, onSaved, selectedTemplateId }) => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const resp = await specialtiesAPI.getTemplates(specialty.id, 'prescriptions');
  ...
  const addNew = () => {
    ...
    const item = { id: tmpId, name: 'Nueva plantilla', content: '', isDefault: templates.length === 0, isActive: true };
    ...
    try {
      ...
      for (const old of db) {
        if (!curById.has(String(old.id))) {
          await specialtiesAPI.deleteTemplate(specialty.id, 'prescriptions', old.id);
        }
      }
      // create/update
      for (const cur of templates) {
        if (String(cur.id).startsWith('new-')) {
          await specialtiesAPI.createTemplate(specialty.id, {
            type: 'prescriptions', name: cur.name, content: cur.content,
          ...
          if (!before || before.name !== cur.name || before.content !== cur.content || before.isDefault !== cur.isDefault || before.isActive !== cur.isActive) {
            await specialtiesAPI.updateTemplate(specialty.id, 'prescriptions', cur.id, {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-11/12 h-5/6 max-w-6xl flex flex-col">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">Plantillas de prescripción - {specialty.name}</h2>
...
export default PrescriptionsTemplateEditor;
